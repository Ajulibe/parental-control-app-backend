...
export class PostCache extends BaseCache {
  ...
  public async savePostToCache(data: ISavePostToCache): Promise<void> {
    ...
    try {
      ...
      //here we are getting the total no. of posts from the redis cache using `users:${currentUserId}` as the key
      const postCount: string[] = await this.client.HMGET(`users:${currentUserId}`, 'postsCount');

      //multi is just a way to avoid always writing this.client
      //instead we now use multi.{ANY METHOD}
